...
GLUtil.createInt8VertexBuffer = function(gl,vertexes) {
	$s.push("GLUtil::createInt8VertexBuffer");
	var $spos = $s.length;
	...
	this.cursorClient.defaultCursor();
	{
		...
		while(_g < _g1.length) {
			...
			if(this.isUnderMouse(viewWorldMatrixButton)) {
				...
				this.gl.uniform4f(this.colorButtonUniform,1,1,1,0.2);
			}
			else {
				button.isActive = false;
				this.gl.uniform4f(this.colorButtonUniform,1,1,1,0.1);
			}
			this.gl.drawArrays(this.gl.TRIANGLES,0,6);
		}
	}
